(function () {
   ...
   */
    function LoadingHttpInterceptor($rootScope, $q) {
      ...
      return {
        request : requestHttpInterceptor,
        response : responseHttpInterceptor,
        responseError : responseErrorHttpInterceptor
      };

      function requestHttpInterceptor(config) {
      ...
      function responseHttpInterceptor(response) {
        if(--loadingCount === 0){
          $rootScope.$broadcast(loadingEventName, {on : false});
        };
        return response;
      };
      function responseErrorHttpInterceptor(response) {
        if(--loadingCount === 0){
          $rootScope.$broadcast(loadingEventName, {on : false});
        }
